...
export default function DropsTabHolders() {
  ...
  const [joinWhitelistHere] = useState(false);
  ...
  useEffect(() => {
    ...
    dispatch(updatePhase(Phase.PRIVATE));
  ...
  const setStartTime = (dropState: IDrop, state: STATE) => {
    if (
      dropState.private &&
      dropState.private.ts_start &&
      dropState.private.ts_end
    ) {
      // start timer with 1 sec period
      const timer = setInterval(() => {
        if (state == STATE.LIVE) {
          setRemainingTime(
            calculateRemainingTime(dropState.private.ts_end! * 1000)
          );
        } else {
          setRemainingTime(
            calculateRemainingTime(dropState.private.ts_start! * 1000)
          );
        }
      }, 1000);
  ...
  const updateState = (dropState: IDrop) => {
    if (
      dropState.private.ts_start &&
      dropState.private.ts_start * 1000 < new Date().getTime()
    ) {
      if (
        dropState.private.ts_end &&
        dropState.private.ts_end * 1000 > new Date().getTime()
      ) {
        setState(STATE.LIVE);
        return STATE.LIVE;
      } else {
      ...
      {state == STATE.PRE && (
        <>
          <Grid container spacing={3}>
            <Grid item sm={12}>
              <Typography sx={styles.dropCountLabel}>
                Whitelist drops stating in
              </Typography>
            </Grid>
          </Grid>
          <Grid container spacing={3}>
            <Grid item md={6}>
              <Card
                sx={[
                  ...
                  styles.dropStatesCard,
                  styles.dropCountdownCard,
                ]}
              >
                <Box
                  sx={[cardStyles.customCardInner, styles.dropStatesCardInner]}
                >
                  <Box sx={styles.dropCountdownList}>
                    <Box sx={styles.dropCountContent}>
                      <Typography sx={styles.dropCountDownValue}>
                        {utils.getDays(remainingTime)} :
                      </Typography>
                      <Typography sx={styles.dropCountDownLabel}>
                        Days
                      </Typography>
                    </Box>
                    <Box sx={styles.dropCountContent}>
            ...
            <Grid item md={3}>
              <Card sx={[cardStyles.customCardUi, styles.dropStatesCard]}>
                ...
                >
                  <Box
                    sx={[styles.dropStatesLabelGroup, styles.dropStatesHeader]}
                  >
                  ...
                  <Box sx={styles.dropStatesLabelGroup}>
                    ...
                    <Typography sx={styles.dropStatesLabelDark}>
                      members
                    </Typography>
                  </Box>
                </Box>
                ...
                {whiteListOnList === false ? (
                  <Box
                    sx={[
                      ...
                      styles.dropStatesCardInner,
                    ]}
                  >
                    <Box
                      sx={[
                        styles.dropStatesLabelGroup,
                        styles.dropStatesHeader,
                      ]}
                    >
            ...
            <Grid item sm={12}>
              {drop && drop.private.ts_start && (
                <Typography sx={styles.dropCountEventDate}>
                  {new Intl.DateTimeFormat("en-us", {
                    dateStyle: "full",
                    timeStyle: "short",
                  }).format(new Date(drop.private.ts_start * 1000))}
                </Typography>
              )}
              <Box sx={styles.earnWhitelistRow}>
                <Box
                  sx={[globalStyles.btnSecondary, styles.earnWhitelistLabel]}
                >
                  ...
                  <>
                    {whiteListOnList === false ? (
                      ...
                      <Button
                        ...
                        color="primary"
                      >
                        <Box sx={globalStyles.btnTextDisabled}>
                          Join Whitelist
                        </Box>
                      </Button>
                    )}
      ...
      {state == STATE.LIVE && (
        <>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item sm={12}>
              {drop && drop.private.ts_end && (
                <Typography sx={styles.dropCountEventDate}>
                  {new Intl.DateTimeFormat("en-us", {
                    dateStyle: "full",
                    timeStyle: "short",
                  }).format(new Date(drop.private.ts_end * 1000))}
                </Typography>
              )}
            </Grid>
          </Grid>
